...
- **Per-repo setup (multi-repo only):** skip entirely for mono-repo — the root file/build above is already sufficient. For each repo in `config.json` `repos[]`, per `reference/tools-setup.md` § "Per-Repo `.code-review-graphignore` (multi-repo only)":
  1. Generate `<repo>/.code-review-graphignore` from that repo's own `repos[].stack` entry — always include `node_modules/` and `dist/` as baseline (regardless of stack), then add stack-derived entries (Node/TypeScript → `coverage/`, `.nyc_output/`; Next.js/frontend → `.next/`, `out/`, `storybook-static/`, `.storybook/`; Motia/iii.dev → `.motia/`, `data/`). Same managed-block pattern as the root file, with a `# --- User entries below ---` footer.
  2. Verify/add `.code-review-graph/` to that repo's own `.gitignore` (not the root workspace one) — append if missing.
